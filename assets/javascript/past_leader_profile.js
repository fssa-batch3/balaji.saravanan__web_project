const pastLeader = JSON.parse(localStorage.getItem("past_leader"));
...
if (found) {
  ...
  position.innerText = `பதவி : ${pastLeader[i].postion}`;
  ...
  position3.innerText = `PARTY : ${pastLeader[i].party_name}`;
  cm_name.append(position3);

  const position4 = document.createElement("p");
  position4.innerText = `அரசியல் :${5}வருடம்`;
