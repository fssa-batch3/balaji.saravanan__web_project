...
const user = users.find((user_v) => user_v.email === profile_check); // i check user profife beacuse this is condition
...
editButton.addEventListener("click", (e) => {
  ...
  select.forEach((select_v) => {
    select_v.disabled = false;
  });
...
saveButton.addEventListener("submit", (e) => {
  // save button function this button save the values of user object
...
deleteButton.addEventListener("click", (e) => {
  // this isdelete button user want delete user pres and account delete
  const index = users.findIndex((user_b) => user_b.email === profile_check); // thisis frofile check and delete

  if (index !== -1) {
    // this is condition not equal to -one
